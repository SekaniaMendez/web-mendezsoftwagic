...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const image = new URL(project.image, origin).toString();
  ...
  return (
    <main className="case-page" style={{ "--project-accent": project.accent, "--project-rgb": project.accentRgb } as CSSProperties}>
      ...
      <section className="case-content">
        ...
        <div className="stack-block reveal"><p>Technology constellation</p><div>{project.technologies.map((technology) => <span key={technology}>{technology}</span>)}</div></div>
        <div className="case-actions reveal">{project.liveUrl ? <a className="button button-primary" href={project.liveUrl} target="_blank" rel="noreferrer">Visit the project <span>↗</span></a> : <span className="button button-quiet">Private development build</span>}<Link className="button button-quiet" href="/#contact">Discuss a project</Link></div>
      </section>
